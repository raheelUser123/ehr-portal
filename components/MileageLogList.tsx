  ...
  return (
    <div className="ml-page">
      <div className="ml-titlebar">
        ...
        <h1>Mileage Log</h1>
        <Link className="ml-create" href="/mileage-log/new">+ Create New</Link>
      </div>
      ...
      <div className="ml-card">
        ...
        <div className="ml-table-wrap"><table className="ml-table"><thead><tr><th>Date</th><th>Resident Initials</th><th>Beginning</th><th>Ending</th><th>Total</th><th>Destination</th><th>Status</th><th>Actions</th></tr></thead><tbody>
          {filtered.map((r) => <tr key={r.id}><td>{r.log_date || "—"}</td><td><strong>{r.resident_initials || "—"}</strong></td><td>{r.beginning_mileage ?? "—"}</td><td>{r.ending_mileage ?? "—"}</td><td>{r.total_mileage ?? "—"}</td><td>{r.destination || "—"}</td><td><span className={`ml-status ${r.status === "submitted" ? "submitted" : "draft"}`}>{r.status || "draft"}</span></td><td><div className="ml-row-actions"><Link href={`/mileage-log/${r.id}/edit`}>Edit</Link><button onClick={() => remove(r.id)}>Delete</button></div></td></tr>)}
        </tbody></table></div>}
      </div>
    </div>
  );
